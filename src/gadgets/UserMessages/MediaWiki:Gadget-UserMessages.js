...
"use strict";
(function () {
    var linktext = wgULS("向该用户发出提醒", "對此使用者發送提醒", null , null ,"對此用戶發送提醒") , nsNr = mw.config.get("wgNamespaceNumber");
    if (nsNr === 3 || nsNr === 2 ||
        nsNr === -1 &&
        ...
        $(function () {
            mw.loader.using(["mediawiki.util"], function () {
                ...
                if ($("#t-AjaxUserMessage").length === 0 && $("#t-AjaxUserMessageLOD").length === 0) {
                    var pHref = mw.util.addPortletLink("p-tb", "#", linktext, "t-AjaxUserMessageLOD", "加载 UserMessages 小工具");
                    if (!pHref) {
