...
$(() => (async () => {
    ...
    $("#mw-content-text > .mw-htmlform-ooui-wrapper").after(`<fieldset><legend class="mw-parser-output">链入情况 | <a href="/Special:Search/${encodeURIComponent(`linksto:"${target}" insource:"${target}"`)}" target="_blank" class="external text">通过搜索查找链入</a></legend><button id="queryWhatlinkshere">点此查询链入情况</button><p id="whatlinkshere" style="display: none;">加载中……</p></fieldset><fieldset><legend class="mw-parser-output">嵌入情况 | <a href="/Special:Search/${encodeURIComponent(`hastemplate:"${target}" insource:"${title}"`)}" target="_blank" class="external text">通过搜索查找嵌入</a></legend><button id="queryWhatembeddedin">点此查询嵌入情况</button><p id="whatembeddedin" style="display: none;">加载中……</p></fieldset>`);
