...
(async () => {
    ...
    function commonBoxs() {
        ...
        const contentParent = $("#mw-content-text")[0] ? $("#mw-content-text") : $("#content"), commonBoxes = contentParent.find(".common-box");
        if (!commonBoxes[0]) {
            return;
        }
    ...
    function searchReferrerJump() {
        if (/\/\/www.(?:google|baidu|bing).com\//.test(document.referrer)) {
            const mobileTags = ["Android", "iPhone", "SymbianOS", "Windows Phone", "iPod"];
            if (!/\sVR\s/g.test(navigator.userAgent)) {
                ...
                const toggleLink = document.querySelector("#mw-mf-display-toggle");
    ...
    function mobileEdit() {
        $(window).on("hashchange", () => {
            ...
            if (/^#\/editor\/(?:new|\d+)$/.test(hash)) {
                ...
                const curNsid = mw.config.get("wgNamespaceNumber");
                const talkNsid = Math.floor(curNsid / 2) * 2 + 1;
                const wgNamespaceIds = mw.config.get("wgNamespaceIds");
                const wgNamespaceNames = Object.keys(wgNamespaceIds);
                let talkNsName;
                for (const i in wgNamespaceNames) {
                    const nsName = wgNamespaceNames[i];
    ...
    //特定系统消息返回 jQuery
    function patchMwMsg() {
        const msgNames = ["mobile-frontend-editor-blocked-info-loggedin"];
        mw.msg = (...args) => {
            const msgName = args[0];
            if (msgNames.includes(msgName)) {
                let message = mw.messages.get(msgName);
    ...
    async function mobilesidebar() {
        ...
        const oouiIcon = (icon, title) => new OO.ui.IconWidget({ icon: icon, title: title }).$element.prop("outerHTML");
        const oouiIndicator = () => new OO.ui.IndicatorWidget({ indicator: "down" }).$element.prop("outerHTML");
        const generateMenuLi = ({ href, text, iconName, className }) => `<li ${className ? `class=${className}` : ""}><a href="${href}" class="mw-ui-icon">${iconName ? oouiIcon(iconName, text) : ""}${text}${className === "mobilesidebar-caption" ? oouiIndicator() : ""}</a></li>`;
        const mobilesidebartimer = setInterval(() => {
            if ($(".menu ul").length > 0 && $(".mobilesidebar-level1").length === 0) {
                ...
                if ([2, 3].includes(wgNamespaceNumber)) {
                    const userName = mw.util.wikiUrlencode(mw.config.get("wgTitle").split("/")[0]);
                    $(".menu > ul:not(.hlist)").last().before([
                ...
                if (wgNamespaceNumber >= 0 && mw.config.get("wgUserGroups").includes("autoconfirmed")) {
                    const pageName = mw.util.wikiUrlencode(mw.config.get("wgPageName"));
                    $(".menu > ul:not(.hlist)").last().before([
                ...
                $(".mobilesidebar-caption").on("click", ({ target }) => {
                    const $ele = $(target).closest(".mobilesidebar-level1");
    ...
    //用户资料页
    if (isUserProfile()) {
        const cardContainer = $(".card-container"), containerImage = cardContainer.find(".card"), containerImageFile = containerImage.find("a.image"), containerImageCaption = containerImage.find(".caption");
