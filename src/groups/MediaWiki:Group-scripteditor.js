/* JS placed here will affect scripteditors only */
/* 函数执行体 */
$(() => {
    //Widget预加载模板
    ...
    if (mw.config.get("wgNamespaceNumber") == 274 && !mw.config.get("wgCurRevisionId")) {
        if (mw.config.get("wgAction") === "edit") {
            ...
            if (regex.test(mw.config.get("wgPageName"))) {
                ...
                location.replace(`${mw.config.get("wgServer") + mw.config.get("wgScriptPath") }/index.php?action=edit&title=${ mw.config.get("wgPageName").replace(/ |_/g, "").replace(/^([^\/]*)[\/\\].*$/i, "$1").split(regex).map((n) => { return toUpperFirstCase(n); }).join("")}`);
                return;
            }
